...
func mig_20260609000000_create_checkout_tables_up(tx *sqlx.Tx) error {
	_, err := tx.Exec(`
		...
		CREATE TABLE IF NOT EXISTS promotion_rules (
			...
			target_product_uuid UUID NOT NULL REFERENCES products(uuid),
			target_sku VARCHAR(50) NOT NULL,
			reward_product_uuid UUID NULL REFERENCES products(uuid),
			reward_sku VARCHAR(50) NULL,
		...
		INSERT INTO promotion_rules (promotion_uuid, promotion_type, target_product_uuid, target_sku, reward_product_uuid, reward_sku, min_quantity, free_quantity, priority)
		SELECT p.uuid, 'FREE_PRODUCT',
			(SELECT pr.uuid FROM products pr WHERE pr.sku = '43N23P'),
			'43N23P',
			(SELECT pr.uuid FROM products pr WHERE pr.sku = '234234'),
			'234234',
			1, 1, 10
		FROM promotions p WHERE p.code = 'MACBOOK_FREE_RASPBERRY'
		ON CONFLICT DO NOTHING;

		INSERT INTO promotion_rules (promotion_uuid, promotion_type, target_product_uuid, target_sku, buy_quantity, pay_quantity, priority)
		SELECT p.uuid, 'BUY_X_PAY_Y',
			(SELECT pr.uuid FROM products pr WHERE pr.sku = '120P90'),
			'120P90',
			3, 2, 20
		FROM promotions p WHERE p.code = 'GOOGLE_HOME_BUY_3_PAY_2'
		ON CONFLICT DO NOTHING;

		INSERT INTO promotion_rules (promotion_uuid, promotion_type, target_product_uuid, target_sku, min_quantity, discount_percentage, priority)
		SELECT p.uuid, 'PERCENTAGE_DISCOUNT',
			(SELECT pr.uuid FROM products pr WHERE pr.sku = 'A304SD'),
			'A304SD',
			3, 10.00, 30
		FROM promotions p WHERE p.code = 'ALEXA_10_PERCENT_DISCOUNT'
		ON CONFLICT DO NOTHING;

		INSERT INTO inventory_movements (product_uuid, sku, movement_type, quantity, stock_before, stock_after, note)
		SELECT uuid, sku, 'INITIAL_STOCK', 10, 0, 10, 'Initial seed stock' FROM products WHERE sku = '120P90'
		UNION ALL
		SELECT uuid, sku, 'INITIAL_STOCK', 5, 0, 5, 'Initial seed stock' FROM products WHERE sku = '43N23P'
		UNION ALL
		SELECT uuid, sku, 'INITIAL_STOCK', 10, 0, 10, 'Initial seed stock' FROM products WHERE sku = 'A304SD'
		UNION ALL
		SELECT uuid, sku, 'INITIAL_STOCK', 2, 0, 2, 'Initial seed stock' FROM products WHERE sku = '234234';
	`)
	return err
}
